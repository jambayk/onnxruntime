...
Missing Doc
  ...
  ONNX_CONTRIB_OPERATOR_SCHEMA(MatMulBnb4)
      ...
      .Attr("quant_type", "Type of quantization used. 1 for FP4, 2 for NF4.", AttributeProto::INT)
      .Attr("double_quant", "Whether absmax is quantized (nested quantization) ot not.", AttributeProto::INT)
      .Attr("nested_blocksize", "Number of groupsize used for nested weight quantization.", AttributeProto::INT, OPTIONAL_VALUE)
      .Input(0, "A", "The input tensor, not quantized.", "T1")
      .Input(1, "B", "Quantized data for weight.", "T2")
      .Input(2, "absmax", "Quantization scale, can be quantized.", "T3")
      .Input(3, "quant_map", "Mapping from quantized value to float value.", "T4")
      .Input(4, "offset", "Offset for nested quantization.", "T4", OpSchema::Optional)
      .Input(5, "nested_absmax", "Quantization scale for nested quantization.", "T4", OpSchema::Optional)
      .Input(6, "nested_quant_map", "Mapping from quantized value to float value for nested quantization.", "T4", OpSchema::Optional)
      .Output(0, "Y", "tensor. The output tensor has the same rank as the input. ", "T1")
      .TypeConstraint("T1", {"tensor(float)", "tensor(float16)"}, "Constrain input and output types to float/half_float tensors.")
      .TypeConstraint("T2", {"tensor(uint8)"}, "Constrain quantized weight types to uint8.")
      .TypeConstraint("T3", {"tensor(uint8)", "tensor(float)"}, "Constrain absmax types to uint8/float. float if nested quantization is used.")
      .TypeConstraint("T4", {"tensor(float)"}, "Constrain quant_map, offset, nested_absmax, nested_quant_map types to float.")
      .TypeAndShapeInferenceFunction([](ONNX_NAMESPACE::InferenceContext& ctx) {
        // Type inference
        propagateElemTypeFromInputToOutput(ctx, 0, 0);
        // Shape inference
