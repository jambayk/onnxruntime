...
void QuantizeBlockwiseBnb4(
  ...
  concurrency::ThreadPool::TryBatchParallelFor(
      ...
      total_block_count,
      [&](ptrdiff_t block_idx) {
        QuantizeBlockBnb4<T, block_size>(src, dst, absmax[block_idx], block_idx, numel);
      },
      0);
}
...
void QuantizeBlockwiseBnb4(
  ...
  if (16 == block_size) {
    ...
    QuantizeBlockwiseBnb4<T, 32>(dst, src, absmax, N, K, thread_pool);
  } else if (64 == block_size) {
    QuantizeBlockwiseBnb4<T, 64>(dst, src, absmax, N, K, thread_pool);
  } else if (128 == block_size) {
    QuantizeBlockwiseBnb4<T, 128>(dst, src, absmax, N, K, thread_pool);
  } else if (256 == block_size) {
    QuantizeBlockwiseBnb4<T, 256>(dst, src, absmax, N, K, thread_pool);
  } else {
...
void DequantizeBlockwiseBnb4(
  ...
  concurrency::ThreadPool::TryBatchParallelFor(
      ...
      total_block_count,
      [&](ptrdiff_t block_idx) {
        DequantizeBlockBnb4<T, block_size>(src, dst, absmax[block_idx], block_idx, numel);
      },
      0);
}
...
void DequantizeBlockwiseBnb4(
  ...
  if (16 == block_size) {
    ...
    DequantizeBlockwiseBnb4<T, 32>(dst, src, absmax, N, K, thread_pool);
  } else if (64 == block_size) {
    DequantizeBlockwiseBnb4<T, 64>(dst, src, absmax, N, K, thread_pool);
  } else if (128 == block_size) {
    DequantizeBlockwiseBnb4<T, 128>(dst, src, absmax, N, K, thread_pool);
  } else if (256 == block_size) {
    DequantizeBlockwiseBnb4<T, 256>(dst, src, absmax, N, K, thread_pool);
  } else {
    ORT_NOT_IMPLEMENTED("only block size 16, 32, 64, 128, 256 are supported.");
  }
}
